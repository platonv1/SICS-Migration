        ...
        public string fn_process(string str_raw, string str_sheet, string str_saved, string str_savef, string str_gender = "", bool boo_open = false, bool boo_clean = false)
        {
            ...
            try 
            {
                for (int intLoop = 1; intLoop <= erawrow + 1; intLoop++)
                {
                    ...
                    if (double.TryParse(str_ISRLife, out double dbl_ISRLife))
                    {
                    ...
                    if (double.TryParse(str_ISRWP, out double dbl_ISRWP))
                    {
                    ...
                    if (double.TryParse(str_ISRADB, out double dbl_ISRADB))
                    {
                    ...
                    if (double.TryParse(str_ISRRider, out double dbl_ISRRider))
                    {
                    ...
                    string str_PremiumDate = wsraw.Cells[intLoop, 28 + int_columnvariation].Text.ToString();//this is column 29 in the raw file
