            ...
            try
            {
                ...
                while (rowcount != erawrow + 2)
                {
                    ...
                    else if (polnum != string.Empty && chck == true)
                    {
                        ...
                        objHlpr.fn_getnamesandlifeID(fullname, dob, out string str_outfname, out string str_outlname, out string str_outlifeid, "098");
                    ...
                    if (wsraw.Cells[prawrow, 14].Text.ToString() != string.Empty)
                    {
                        osa = double.Parse(wsraw.Cells[prawrow, 14].Text.ToString()) / double.Parse(wsraw.Cells[prawrow, 25].Text.ToString());
                        isar = double.Parse(wsraw.Cells[prawrow, 21].Text.ToString()) / double.Parse(wsraw.Cells[prawrow, 25].Text.ToString());
                        effyear = int.Parse(eff.Substring(eff.Length - 4, 4)) - int.Parse(wsraw.Cells[prawrow, 8].Text.ToString()) ;
                        prem = String.Format("{0:0.000}", double.Parse(wsraw.Cells[prawrow, 27].Text.ToString()) * .10);
                    }
                ...
                objdt_template.Rows.Add(dtworkRow);
                #endregion
                string despath = str_saved + @"\BM098-" + str_savef + ".xlsx";
