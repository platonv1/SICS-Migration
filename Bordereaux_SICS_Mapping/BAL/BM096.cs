    ...
    class BM096
    {
        public string fn_process(string str_raw, string str_sheet, string str_saved, string str_savef, string str_gender = "", bool boo_open = false, bool boo_clean = false)
        {
            ...
            try
            {
                ...
                Microsoft.Office.Interop.Excel.Worksheet wsraw_ = wbraw.Sheets["SUMMARY"];
                ...
                while (rowcount != erawrow + 2)
                {
                    ...
                    plan = wsraw.Cells[prawrow, 12].Text.ToString();
                    eff = wsraw.Cells[prawrow, 20].Text.ToString();
                    ...
                    if (wsraw.Cells[prawrow, 8].Text.ToString() != string.Empty)
                    {
                        osa = double.Parse(wsraw.Cells[prawrow, 8].Text.ToString());
                        isar = double.Parse(wsraw.Cells[prawrow, 8].Text.ToString());
                        effyear = int.Parse(eff.Substring(eff.Length - 4, 4)) - int.Parse(wsraw.Cells[prawrow, 18].Text.ToString()) ;
                        prem = String.Format("{0:0.000}", (double.Parse(wsraw.Cells[prawrow, 25].Text.ToString()) * .10) 
                            + (double.Parse(wsraw.Cells[prawrow, 26].Text.ToString()) * .10)
                            + (double.Parse(wsraw.Cells[prawrow, 27].Text.ToString()) * .10));
                    }
                    ed = string.Empty;
                    ia = wsraw.Cells[prawrow, 18].Text.ToString();
                    rowcount++;
                }
                #endregion
                ...
                objdt_template.Rows.Add(dtworkRow);
                #endregion
                string despath = str_saved + @"\BM096-" + str_savef + ".xlsx";
