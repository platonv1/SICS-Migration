    ...
    class BM015_B
    {
        public string fn_process(string str_raw, string str_sheet, string str_saved, string str_savef, string str_gender = "", bool boo_open = false, bool boo_clean = false)
        {
            ...
            try
            {
                ...
                string pref = wsraw.Cells[prawrow, 8].Text.ToString();
                string polnum = wsraw.Cells[prawrow, 10].Text.ToString();
                string cert = wsraw.Cells[prawrow, 10].Text.ToString();
                string dob = wsraw.Cells[prawrow, 11].Text.ToString();
                string poldate = wsraw.Cells[prawrow, 12].Text.ToString();
                string age = wsraw.Cells[prawrow, 13].Text.ToString();
                string retention = wsraw.Cells[prawrow, 15].Text.ToString();
                string reins = wsraw.Cells[prawrow, 16].Text.ToString();
                string premium = wsraw.Cells[prawrow, 18].Text.ToString();

                string rider = wsraw.Cells[prawrow, 9].Text.ToString();
                string rprem = wsraw.Cells[prawrow, 19].Text.ToString();
                ...
                while (rowcount != erawrow + 2)
                {
                    ...
                    if (cession.ToUpper().IndexOf("G-") == 0)
                    {
                        chck = true;
                    }
                    else
                    ...
                    if (cession != string.Empty && chck == true)
                    {
                        ...
                        _var.dtworkRow01[26] = reins;
                        _var.dtworkRow01[27] = reins;
                        _var.dtworkRow01[77] = reins;
                        ...
                        if (str_sheet.ToUpper().Contains("FIRST"))
                        {
                            ...
                            _var.dtworkRow01[57] = Math.Round(double.Parse(premium.ToString()) * mul, 2); 
                        }
                        else if (str_sheet.ToUpper().Contains("RENEWAL"))
                        {
                            _var.dtworkRow01[21] = "TRENEW";
                            _var.dtworkRow01[58] = "4001";
                            _var.dtworkRow01[59] = Math.Round(double.Parse(premium.ToString()) * mul, 2); 
                        }
                        else if (str_sheet.ToUpper().Contains("ADJ FY"))
                        {
                            _var.dtworkRow01[21] = "ADJUST";
                            _var.dtworkRow01[60] = "4002";
                            _var.dtworkRow01[61] = Math.Round(double.Parse(premium.ToString()) * mul, 2);
                        }
                        else if (str_sheet.ToUpper().Contains("ADJ REN"))
                        {
                            _var.dtworkRow01[21] = "ADJUST";
                            _var.dtworkRow01[62] = "4004";
                            _var.dtworkRow01[63] = Math.Round(double.Parse(premium.ToString()) * mul, 2); 
                        }
                        ...
                        {
                            DateTime parsedreins = Convert.ToDateTime(poldate);
                        ...
                        _var.dtworkRow01[25] = Math.Round(double.Parse(_var.dtworkRow01[25].ToString()) * mul, 2);
                        _var.dtworkRow01[27] = Math.Round(double.Parse(_var.dtworkRow01[27].ToString()) * mul, 2);
                        _var.dtworkRow01[77] = Math.Round(double.Parse(_var.dtworkRow01[77].ToString()) * mul, 2);
                        #endregion
                        ...
                        if (!string.IsNullOrEmpty(rider))
                        {
                            ...
                            _var.dtworkRow02 [59] = "0";

                            if (str_sheet.ToUpper() == ("FIRST YEAR"))
                            {
                                _var.dtworkRow02[57] = Math.Round(double.Parse(rprem.ToString()) * mul, 2);
                            }
                            else if (str_sheet.ToUpper() == ("RENEWAL"))
                            {
                                _var.dtworkRow02[59] = Math.Round(double.Parse(rprem.ToString()) * mul, 2);
                            }
                            else if (str_sheet.ToUpper() == ("ADJUSTMENT FIRST YEAR"))
                            {
                                _var.dtworkRow02[61] = Math.Round(double.Parse(rprem.ToString()) * mul, 2);
                            }
                            else if (str_sheet.ToUpper().Contains("ADJUSTMENT RENEWAL"))
                            {
                                _var.dtworkRow02[63] = Math.Round(double.Parse(rprem.ToString()) * mul, 2);
                            }
                    ...
                    pref = wsraw.Cells[prawrow, 8].Text.ToString();
                    polnum = wsraw.Cells[prawrow, 10].Text.ToString();
                    cert = wsraw.Cells[prawrow, 10].Text.ToString();
                    dob = wsraw.Cells[prawrow, 11].Text.ToString();
                    poldate = wsraw.Cells[prawrow, 12].Text.ToString();
                    age = wsraw.Cells[prawrow, 13].Text.ToString();
                    retention = wsraw.Cells[prawrow, 15].Text.ToString();
                    reins = wsraw.Cells[prawrow, 16].Text.ToString();
                    premium = wsraw.Cells[prawrow, 18].Text.ToString();

                    rider = wsraw.Cells[prawrow, 9].Text.ToString();
                    rprem = wsraw.Cells[prawrow, 19].Text.ToString();
                    ...
                    if ((!String.IsNullOrEmpty(poldate)) && (poldate.Length >= 4))
                    {
                        poldate = poldate.Trim();
                        iyear = poldate.Substring(poldate.Length - 4, 4);
                    }
                ...
                string despath = str_saved + @"\BM015-" + str_savef + ".xlsx";
